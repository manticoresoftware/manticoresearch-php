...
class SqlToArray extends Response
{
    public function getResponse()
    {
        ...
        // workaround for the change in Manticore Search made in 4.2.1, after which any query in mode=raw returns an array
        if (is_array($response) and count($response) === 1 and isset($response[0]['columns'], $response[0]['data'])) {
            $response['columns'] = $response[0]['columns']; 
