...
use Manticoresearch\Query\QueryMatch;
...
class SearchTest extends TestCase
{
    ...
    {
        $q = new BoolQuery();
        $q->must(new QueryMatch(['query' => 'team of explorers', 'operator' => 'and'], '*'));
        $result = self::$search->search($q)->get();
        $this->assertCount(3, $result);
    }
    ...
    {
        $q = new BoolQuery();
        $q->should(new QueryMatch(['query' => 'team of explorers', 'operator' => 'and'], '*'));
        $result = self::$search->search($q)->get();
        $this->assertCount(3, $result);
    }
    ...
    {
        $q = new BoolQuery();
        $q->must(new QueryMatch(['query' => 'team of explorers', 'operator' => 'or'], '*'));
